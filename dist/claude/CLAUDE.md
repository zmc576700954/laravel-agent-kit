# Laravel Agent Rules（Claude Code）
...
Claude Code 入口为单文件规则，不假设存在 skills 体系。需要按“场景闭环”工作时，按下列口径手工路由，并以对应闭环输出（Scope/Task Level/Plan/Implement/Verify）交付：
...
## Review Checklist（交付前自检要点）

- 已明确 In Scope / Out of Scope，且未扫描整个项目
- 已按轻/中/重分级并匹配记录强度
- 若触发 Stop Conditions 已先停下追问确认
- 交付包含 Verify 命令或最小验证清单
- 变更遵循最小改动原则，不顺手重构/清理/抽象升级
